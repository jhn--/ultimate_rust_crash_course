...
fn main() {
    ...
    let handle = thread::spawn(move || {
        expensive_sum(my_vector) // either not use ; or use the return keyword(w/ ;)
        // why is there a need to "return"?
    });
    ...
    // let sum = match handle.join().unwrap() {
    // // this is wrong, i got () too and i thought i need to extract OK() from it.

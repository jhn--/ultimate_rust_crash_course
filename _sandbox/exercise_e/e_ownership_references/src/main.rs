...
fn inspect(s: &String) -> bool {
    if s.ends_with("s") {
        println!("Contents of {} is plural.", s);
        true
    } else {
        println!("Contents of {} is singular.", s);
        false
    }
}

fn change(s: &mut String) {
    // just gonna reuse inspect(), so some out-of-scope modifications done to inspect()
    if !inspect(s) {
        s.push_str("s");
    }
}
...
fn eat(s: String) -> bool {
    ...
    // oh man i can simplify lines 68-72 to just -
